    ...
    public function addEntryPointAssets(string $entrypoint): void
    {
        ...
        if ($this->entrypoints === []) {
            $this->entrypoints = json_decode(
                file_get_contents($this->media_directory_path . '/entrypoints.json'),
        ...
        if (!array_key_exists($entrypoint, $this->entrypoints) || $this->entrypoints[$entrypoint] === []) {
            throw new RuntimeException(
                "Could not find the entry point named [$entrypoint] in [media/$this->media_directory/entrypoints.json]!",
                500
            );
        }
        ...
        if (array_key_exists('css', $this->entrypoints[$entrypoint])) {
            foreach ($this->entrypoints[$entrypoint]['css'] as $url) {
                $this->assetsManager->registerAndUseStyle(
                    $presetPrefix . pathinfo($url, PATHINFO_FILENAME),
        ...
        if (array_key_exists('js', $this->entrypoints[$entrypoint])) {
            foreach ($this->entrypoints[$entrypoint]['js'] as $url) {
                $this->assetsManager->registerAndUseScript(
                    $presetPrefix . pathinfo($url, PATHINFO_FILENAME),
                    Uri::root(true) . ltrim($url, '/')
                );
            }
    ...
    public function getAssetUrl(string $filename): string
    {
        ...
        if ($this->manifest === []) {
            $this->manifest = json_decode(
                file_get_contents($this->media_directory_path . '/manifest.json'),
                JSON_OBJECT_AS_ARRAY,
                512,
                JSON_THROW_ON_ERROR
            );
